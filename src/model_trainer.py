...
def strategize(ticker, indicator_list=['RSI', 'MACD', 'BBAND'], start=datetime.datetime.now()-datetime.timedelta(days=10*365), end=datetime.datetime.today(), train_test_split=100):
    ...
    signal = 1
    best_strategy = EasyDict(
        {'Indicator': 'None', 'Returns': -1, 'Unrealized Gains': -1, 'Trade Count': -1})
    for indicator in indicator_list:
        ...
        cfg, stock, model = model_utils.model_constructor(
            ticker, signal, abs_returns, indicator, train_test_split=train_test_split, start=start, end=end)

        ret, uret, tc, avgDays, buy_signals, sell_signals = model(cfg, stock)

        strategy_returns = EasyDict(
            {'Indicator': indicator, 'Returns': ret, 'Unrealized Gains': uret, 'Trade Count': tc})
    ...
    strategy_filepath = str(Path(os.path.dirname(
        __file__))) + '\\stock_data\\' + stock.Name + r'\strategy\best_strategy.pickle'
    try:
